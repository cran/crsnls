...
#' @param N (optional) size of population
#' @param my_eps (optional) is used for stopping condition
#' @param max_evals (optional) is used for stopping condition, specifies maximum number of objective function evaluations per dimension (dimension=nonlinear model parameter)
#' @param delta (optional) controls the competition of local heuristics
#' @param w0 (optional) controls the competition of local heuristics
...
#' mod
#'
crs4hc <- function(formula, data , a, b, N, my_eps=1e-15, max_evals=40000, delta=0.05, w0=0.5)
{
  ...
  P <- matrix(data = 0, nrow = N, ncol = dim+1)
  ...
  tss_part <- data[,as.character(resVarName)] - mean(data[,as.character(resVarName)])
  tss <- sum(tss_part^2)
  ...
  while ((fmax-fmin > tss_myeps) && (num_of_evals < dim*max_evals))
  {
    ...
    switch(hh,
           ...
           y <- decrs_radp(P,N,dim,c(0.4,0.9,fmin,fmax))
           )
    switch(hh,
  ...
  result$algorithmInfo$heurUses <- c(REFL1 = refl1Uses, REFL25 = refl25Uses, REFLB = reflbUses, CDEADP = cdeadpUses)
  result$algorithmInfo$numOfEvals <- num_of_evals
  result$algorithmInfo$numOfResets <- nrst
  result$data <- data
  result$other$RHS <- RHS
  result$other$varNamesRHS <- varNamesRHS[!is.na(match(varNamesRHS, colnames(data)))]
...
#' @description
#' This function estimates the regression coefficients of a nonlinear regression function using least squares.
#' The minimization is performed by the CRS algorithm with four competing local heuristics and adaptive stopping condition. Algorithm is described in Tvrdík, Křivý and Mišík (2007).
#'
#' @param formula (obligatory) a nonlinear \link{formula} including variables and parameters
#' @param data (obligatory) data frame in which to evaluate the variables in \code{formula}
#' @param a (obligatory) a vector of length equal to number of parameters representing lower bounds of search space (bounds for parameters must be specified in the same order they appear on right-hand side of \code{formula})
#' @param b (obligatory) a vector of length equal to number of parameters representing upper bounds of search space (bounds for parameters must be specified in the same order they appear on right-hand side of \code{formula})
#' @param N (optional) size of population
#' @param my_eps0 (optional) is used for adaptation of stopping condition
#' @param gamma (optional) is used for adaptation of stopping condition
#' @param max_evals (optional) is used for stopping condition, specifies maximum number of objective function evaluations per dimension (dimension=nonlinear model parameter)
#' @param delta (optional) controls the competition of local heuristics
#' @param w0 (optional) controls the competition of local heuristics
...
#' mod
#'
crs4hce <- function(formula, data , a, b, N, my_eps0=10e-9, gamma=10e7, max_evals=40000, delta=0.05, w0=0.5)
{
  if (missing(formula) || missing(data) || missing(a) || missing(b))
  {
    stop("One or more obligatory parameters are missing.")
...
decrs_radp <- function(P, N, d, vecpar)
{
  Fmin <- vecpar[1]
  CR <- vecpar[2]
  fmin <- vecpar[3]
...
zrcad <- function(y,a,b)
{
  ...
  for (i in zrc)
  {
    ...
    {
      if (y[i]>b[i]) y[i]<-2*b[i]-y[i]
      else
      {
        if (y[i]<a[i]) y[i]<-2*a[i]-y[i]
      }
    }
  }
  return(y)
...
plot.crs4hc <- function(x, ...)
{
    ...
    {
      indVarName <- x$other$varNamesRHS[i]
      plot(df[,as.character(indVarName)], df[,as.character(depVarName)], type = "l", xlab = indVarName, ylab=depVarName)
      points(x=x$data[,as.character(indVarName)],y=x$data[,as.character(depVarName)], col="red")
    }
    par(mfrow=c(1,1))
}
